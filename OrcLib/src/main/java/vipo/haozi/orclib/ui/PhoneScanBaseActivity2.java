...
import vipo.haozi.orclib.task.RecogThread2;
...
public class PhoneScanBaseActivity2 extends AppCompatActivity implements SurfaceHolder.Callback, Camera.PreviewCallback, View.OnClickListener,SensorEventListener {
    ...
    /**识别进程*/
    private RecogThread2 recogThread;
    /**自动对焦计时器*/
    ...
    private void startRecog(byte[] data, Camera camera){
        if(recogThread == null){
            //初始化识别进程
            recogThread = new RecogThread2(camera, iv_camera_scanarea, new RecogTaskListener() {
                @Override
